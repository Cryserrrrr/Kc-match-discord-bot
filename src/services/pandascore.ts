...
export class PandaScoreService {
  ...
  async getLiveKarmineCorpMatches(): Promise<PandaScoreMatch[]> {
    const kcTeamIds = [
      TEAM_IDS.LOL.KC,
      TEAM_IDS.LOL.KCB,
      TEAM_IDS.LOL.KCBS,
    ...
    try {
      logger.info(
        `Fetching live matches for KC teams: ${kcTeamIds.join(", ")}`
      );
  ...
  async getFinishedKarmineCorpMatches(): Promise<PandaScoreMatch[]> {
    const kcTeamIds = [
      TEAM_IDS.LOL.KC,
      TEAM_IDS.LOL.KCB,
      TEAM_IDS.LOL.KCBS,
    ...
    try {
      logger.info(
        `Fetching finished matches for KC teams: ${kcTeamIds.join(", ")}`
      );
      ...
      logger.info(
        `Found ${karmineMatches.length} finished Karmine Corp matches`
      );
      return karmineMatches;
    } catch (error) {
      logger.error("Error fetching finished Karmine Corp matches:", error);
      throw error;
    }
  }
